...
const calculateHash = (index: number, previousHash: string, timestamp: number, data: Transaction[], difficutly: number, nonce: number): string =>
	CryptoJS.SHA256(index + previousHash + timestamp + data + difficutly + nonce).toString();
...
const hasValidHash = (block: Block): boolean => {
	...
	if(!hashMatchesDifficulty(block.hash, block.difficulty)) {
		console.log('block difficutly not satisfied. Expected: ' + block.difficulty + 'got: ' + block.hash);
	}
	return true;
}
